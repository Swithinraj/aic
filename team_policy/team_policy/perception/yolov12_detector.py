...
class CanonicalDetection:
    ...
    def from_raw(cls, raw: Dict, family: str, camera_name: str) -> "CanonicalDetection":
        ...
        center_raw = raw.get("center_uv")
        if isinstance(center_raw, list) and len(center_raw) >= 2:
            center = np.asarray(center_raw[:2], dtype=np.float64).reshape(2)
        else:
...
class CanonicalTrack:
    ...
    def _servo_anchor_fields(self) -> Dict:
        ...
        geom_source = "obb_center" if "obb_corners_uv" in self.extra else source
        common = {
        ...
        if self.family in {"sfp_port", "sc_port"}:
            fields = {
                **common,
                ...
                # The SFP port line is intentionally the centered long axis,
                # not an outer edge.  This is the visual-servo reference.
                "alignment_line_role": "port_mouth",
                "alignment_line_uv": [[float(v) for v in left.tolist()], [float(v) for v in right.tolist()]],
                "alignment_line_mid_uv": [float(v) for v in center.tolist()],
                "alignment_line_angle_rad": float(angle),
                "alignment_line_source": geom_source,
            }
            ...
            fields.update(
                {
                    ...
                    "alignment_line_uv": [
                        [float(v) for v in front_left.tolist()],
                        [float(v) for v in front_right.tolist()],
                    ],
                    "alignment_line_mid_uv": [float(v) for v in front_center.tolist()],
                    "alignment_line_angle_rad": float(front_angle),
                    "alignment_line_source": geom_source,
                }
            )
...
class PerCameraCanonicalTracker:
    ...
    def update(self, detections: List[CanonicalDetection], now: float, frame_bgr: Optional[np.ndarray] = None) -> List[Dict]:
        ...
        for det in detections:
            if det.confidence >= self.owner._family_track_low_conf(det.family):
        ...
        for family in _FAMILY_ORDER:
            ...
            for t_idx, d_idx in matches:
                ...
                track.update(
                    ...
                    self.owner._family_confirm_hits(track.family),
        ...
        for det in detections:
            if id(det) in matched_det_ids:
                continue
            if det.confidence < self.owner._family_track_new_conf(det.family):
                continue
            ...
            track.confirmed = track.hit_count >= self.owner._family_confirm_hits(det.family)
            self._next_track_id += 1
...
class YoloV12MultiCameraDetector(Node):
    ...
    def __init__(self):
        ...
        self._raw_conf = float(os.environ.get("YOLOV12_RAW_CONF", "0.15"))
        ...
        self.task_board_publish_conf = param_env_float("task_board_publish_conf", "YOLOV12_TASK_BOARD_PUBLISH_CONF", 0.35)
        self.port_publish_conf = param_env_float("port_publish_conf", "YOLOV12_PORT_PUBLISH_CONF", 0.45)
        self.module_publish_conf = param_env_float("module_publish_conf", "YOLOV12_MODULE_PUBLISH_CONF", 0.20)
        ...
        self.draw_alignment_debug = self._env_bool("YOLOV12_DRAW_ALIGNMENT_LINES", True)
        ...
        self._max_hz = max(0.1, float(os.environ.get("YOLOV12_MAX_HZ", "20.0")))
        ...
        self._family_aliases = {
            ...
            "sfp_module": self._parse_name_set(
                os.environ.get(
                    ...
                    "sfp_module,sfp module,sfp-module,sfpmodule,transceiver,sfp_plug,sfp plug,sfp_connector,sfp connector,sfp_tip",
                )
            ),
    ...
    def _family_track_new_conf(self, family: str) -> float:
        if family == "task_board":
            return 0.25
        if family in {"sfp_port", "sc_port"}:
            return 0.25
    ...
    @staticmethod
    def _class_name_from_names(names, cls_idx: int) -> str:
        try:
            idx = int(cls_idx)
    ...
    @staticmethod
    def _clip_bbox_to_image(bbox: np.ndarray, width: int, height: int) -> np.ndarray:
        ...
        out[2] = np.clip(out[2], 1, max(1, width))
        out[3] = np.clip(out[3], 1, max(1, height))
        if out[2] <= out[0] + 1.0:
            out[2] = min(float(width), out[0] + 2.0)
        if out[3] <= out[1] + 1.0:
            out[3] = min(float(height), out[1] + 2.0)
        return out
    ...
    def _infer_raw_detections(self, frame: np.ndarray, msg: Image, camera_name: str) -> List[Dict]:
        ...
        if obb is not None and getattr(obb, "data", None) is not None and len(obb) > 0:
            corners_all = self._to_numpy(getattr(obb, "xyxyxyxy", None))
            xywhr_all = self._to_numpy(getattr(obb, "xywhr", None))
            xyxy_all = self._to_numpy(getattr(obb, "xyxy", None))
            confs = self._to_numpy(getattr(obb, "conf", None))
            clss = self._to_numpy(getattr(obb, "cls", None))
            ids = self._to_numpy(getattr(obb, "id", None))
            if confs is not None and clss is not None:
                for idx, (conf, cls_idx) in enumerate(zip(confs, clss.astype(int))):
                    cls_name = self._class_name_from_names(names, int(cls_idx))
                    corners = None
                    if corners_all is not None and idx < len(corners_all):
                        ...
                        continue
                    bbox = self._clip_bbox_to_image(bbox, img_w, img_h)
                    extra = {}
        ...
        for idx, (box, conf, cls_idx) in enumerate(zip(xyxy, confs, clss)):
            cls_name = self._class_name_from_names(names, int(cls_idx))
            bbox = self._clip_bbox_to_image(np.asarray(box, dtype=np.float64), img_w, img_h)
            det = {
                "class_id": int(cls_idx),
                "raw_class_name": cls_name,
                "base_class_name": cls_name,
    ...
    def _draw_alignment_lines(self, image: np.ndarray, detections: List[Dict]) -> np.ndarray:
        ...
        for det in detections:
            ...
            if isinstance(mid_uv, list) and len(mid_uv) >= 2:
                try:
                    mid = tuple(np.round(np.asarray(mid_uv[:2], dtype=np.float32)).astype(int).tolist())
                except Exception:
                    mid = None
            else:
                mid = (int(round((p0[0] + p1[0]) * 0.5)), int(round((p0[1] + p1[1]) * 0.5)))
            if mid is None:
                continue
            cv2.circle(out, mid, 6, (0, 0, 0), -1, cv2.LINE_AA)
            cv2.circle(out, mid, 4, (0, 255, 0), -1, cv2.LINE_AA)
            role = str(det.get("alignment_line_role", "")).strip().lower()
            label = "PT" if role == "plug_tip" else "PM" if role == "port_mouth" else role[:4]
            if label:
                cv2.putText(out, label, (mid[0] + 7, mid[1] - 7), cv2.FONT_HERSHEY_SIMPLEX, 0.45, (0, 0, 0), 3, cv2.LINE_AA)
                cv2.putText(out, label, (mid[0] + 7, mid[1] - 7), cv2.FONT_HERSHEY_SIMPLEX, 0.45, (0, 255, 0), 1, cv2.LINE_AA)
        return out
    ...
    def _set_alignment_line(self, det: Dict, role: str, line_uv: np.ndarray, source: str) -> None:
        ...
        det["alignment_line_angle_rad"] = float(np.arctan2(float(vec[1]), float(vec[0])))
