...
# Print the exact line number and command if anything causes an unexpected exit
...
SESSIONS_DIR="/home/ujjwal/ws_aic/src/aic/team_policy/team_policy/training_robot/configs/sessions"
...
kill_lingering() {
    ...
    local model_pids
    model_pids=$(pgrep -f "aic_model" 2>/dev/null) || model_pids=""
    if [[ -n "$model_pids" ]]; then
        kill -SIGKILL $model_pids 2>/dev/null || true
    fi
}
...
log_scores() {
    ...
    local found=0
    local score_lines
    score_lines=$(grep -E "Score:" "$status_file" 2>/dev/null) || score_lines=""
    ...
    if [ "$found" -eq 0 ]; then
        echo "$(ts) [WARN] No score lines parsed for ${run}."
    fi
    return 0
}
