...
# SFP plug/port line convergence.  Angle error is scaled into pixel units so
# the local Jacobian can solve midpoint and angle with one pseudo-inverse.
_SFP_LINE_ALIGN_TOL_PX = 6.0
_SFP_LINE_ALIGN_TOL_RAD = 0.08
_SFP_LINE_ALIGN_ANGLE_WEIGHT_PX = 45.0
_SFP_LINE_ALIGN_PROBE_ROT_RAD = 0.015
_SFP_LINE_ALIGN_MAX_ROT_STEP_RAD = 0.035
_SFP_LINE_ALIGN_MAX_TOTAL_ROT_RAD = 0.25
...
class RunACT(Policy):
    def __init__(self, parent_node: Node):
        ...
        self._cam_last_port_line: dict[str, Optional[dict]] = {cam: None for cam in _CAMERAS}
        self._cam_last_plug_line: dict[str, Optional[dict]] = {cam: None for cam in _CAMERAS}
        # First-sight locked port line per camera.  Once set it is never
        # overwritten, so mis-classifications after the port disappears during
        # insertion cannot corrupt the convergence reference.
        self._locked_port_line: dict[str, Optional[dict]] = {cam: None for cam in _CAMERAS}
    ...
    @staticmethod
    def _line_angle_mod_pi(angle: float) -> float:
    ...
    def _parse_alignment_line(cls, det: dict, role: str, now: float) -> Optional[dict]:
        ...
        if not isinstance(line, list) or len(line) != 2:
            return None
        try:
            arr = np.asarray(line, dtype=np.float64).reshape(2, 2)
            if not np.all(np.isfinite(arr)):
                return None
            mid_val = det.get("alignment_line_mid_uv")
            if isinstance(mid_val, list) and len(mid_val) == 2:
                mid = np.asarray(mid_val, dtype=np.float64).reshape(2)
            else:
                mid = 0.5 * (arr[0] + arr[1])
            angle_val = det.get("alignment_line_angle_rad")
    ...
    def _camera_sfp_line_measurement(self, cam: str, img_msg=None) -> Optional[dict]:
        now = time.time()
        with self._cam_lock:
            locked_port = self._locked_port_line.get(cam)
            # Prefer the locked first-sight port line; fall back to last seen.
            port_line = locked_port if locked_port is not None else self._cam_last_port_line.get(cam)
            plug_line = self._cam_last_plug_line.get(cam)
        if port_line is None or plug_line is None:
            return None
        plug_age = now - float(plug_line.get("stamp", 0.0))
        if plug_age > _PIXEL_ALIGN_FRESH_S:
            return None
        ...
        port_mid = np.asarray(port_line["mid"], dtype=np.float64).reshape(2)
        plug_mid = np.asarray(plug_line["mid"], dtype=np.float64).reshape(2)
        midpoint_error = plug_mid - port_mid
        angle_error = self._line_angle_error(float(plug_line["angle"]), float(port_line["angle"]))
        weighted_error = np.array(
            [
                midpoint_error[0],
                midpoint_error[1],
                angle_error * _SFP_LINE_ALIGN_ANGLE_WEIGHT_PX,
            ],
            dtype=np.float64,
        )
        return {
            ...
            "err_px": float(np.linalg.norm(midpoint_error)),
            "port_age": port_age,
            "plug_age": plug_age,
        }

    def _best_sfp_line_measurement(self, obs_msg=None) -> Optional[dict]:
        for cam in _PIXEL_ALIGN_CAMERA_ORDER:
            img_msg = getattr(obs_msg, f"{cam}_image", None) if obs_msg is not None else None
            meas = self._camera_sfp_line_measurement(cam, img_msg)
            if meas is not None:
                return meas
        return None
    ...
    def _cb_per_camera_yolo(self, msg: String, cam: str) -> None:
        ...
        best_port_line_rank = None
        best_port_line_conf = float("-inf")
        best_port_line: Optional[dict] = None
        best_plug_line_conf = float("-inf")
        best_plug_line: Optional[dict] = None
        now = time.time()
        for det in dets:
            if not isinstance(det, dict):
                continue
            ...
            if bbox is not None and len(bbox) == 4:
                try:
                    ...
                    bbox_f = None
                if bbox_f is not None and (
                    best_rank is None or rank < best_rank or (rank == best_rank and conf > best_conf)
                ):
                    best_rank = rank
                    best_conf = conf
                    best_bbox = bbox_f
            ...
            if role == "port_mouth":
                line = self._parse_alignment_line(det, "port_mouth", now)
                if line is not None and (
                    best_port_line_rank is None
                    or rank < best_port_line_rank
                    or (rank == best_port_line_rank and conf > best_port_line_conf)
                ):
                    best_port_line_rank = rank
                    best_port_line_conf = conf
                    best_port_line = line

        if best_bbox is not None:
            with self._cam_lock:
                self._cam_last_det_time[cam] = now
                self._cam_last_conf[cam] = best_conf
                self._cam_last_bbox[cam] = best_bbox
        if best_port_line is not None or best_plug_line is not None:
            with self._cam_lock:
                if best_port_line is not None:
                    self._cam_last_port_line[cam] = best_port_line
                    # First-sight lock: keep the very first valid port line as
                    # the reference so mis-classifications after the port
                    # disappears (e.g. during insertion) cannot overwrite it.
                    if self._locked_port_line.get(cam) is None:
                        self._locked_port_line[cam] = best_port_line
                if best_plug_line is not None:
                    self._cam_last_plug_line[cam] = best_plug_line
    ...
    def _reset_task_target(self, task: Task) -> None:
        ...
        with self._cam_lock:
            for cam in _CAMERAS:
                ...
                self._cam_last_port_line[cam] = None
                self._cam_last_plug_line[cam] = None
                self._locked_port_line[cam] = None
    ...
    # ------------------------------------------------------------------
    # Phase 1.6: SFP plug/port line convergence before insertion
    # ------------------------------------------------------------------

    def _send_relative_line_delta(
        self,
        get_observation,
        move_robot,
        dx: float,
        ...
        pos, quat, force_n, torque_n = self._get_tcp_state(obs_msg)
        if force_n > self.force_hard_stop_n or torque_n > self.torque_hard_stop_nm:
            self.get_logger().error(f"SFP LINE HARD STOP — F={force_n:.1f}N T={torque_n:.2f}Nm")
            return None, pos, quat

        axis = np.asarray(rotation_axis, dtype=np.float64).reshape(3)
        axis_norm = float(np.linalg.norm(axis))
        if axis_norm < 1e-9:
            axis = _quat_to_rotation_matrix(quat)[:, 2]
            axis_norm = float(np.linalg.norm(axis))
        axis = axis / max(axis_norm, 1e-9)

        target_pos = pos + np.array([float(dx), float(dy), 0.0], dtype=np.float64)
        dq = _axis_angle_to_quat(axis * float(dtheta))
        target_quat = _quat_multiply(dq, quat)
        nrm = float(np.linalg.norm(target_quat))
        if nrm > 1e-9:
            target_quat /= nrm
        target_pose = _pose_from_arrays(target_pos, target_quat)
        ...
        obs_msg = get_observation()
        if obs_msg is None:
            return None, target_pos, target_quat
        pos, quat, _, _ = self._get_tcp_state(obs_msg)
        return obs_msg, pos, quat

    def _estimate_sfp_line_jacobian(
        self,
        get_observation,
        move_robot,
        camera: str,
        ...
        probes = (
            (_PIXEL_ALIGN_PROBE_M, 0.0, 0.0, _PIXEL_ALIGN_PROBE_M),
            (0.0, _PIXEL_ALIGN_PROBE_M, 0.0, _PIXEL_ALIGN_PROBE_M),
            (0.0, 0.0, _SFP_LINE_ALIGN_PROBE_ROT_RAD, _SFP_LINE_ALIGN_PROBE_ROT_RAD),
        )
        cols = []
        for axis_idx, (dx, dy, dtheta, denom) in enumerate(probes):
            obs_probe, _, _ = self._send_relative_line_delta(
                get_observation, move_robot, dx, dy, dtheta, rotation_axis, start_time
            )
            if obs_probe is None:
                return None
            meas_probe = self._camera_sfp_line_measurement(camera, getattr(obs_probe, f"{camera}_image", None))
            self._send_relative_line_delta(
                get_observation, move_robot, -dx, -dy, -dtheta, rotation_axis, start_time
            )
            if meas_probe is None:
                return None
            col = (meas_probe["weighted_error"] - base_error) / float(denom)
            if float(np.linalg.norm(col)) < 1e-6:
                return None
            cols.append(col)
            self.get_logger().info(
                "SFP_LINE_ALIGN probe "
                f"axis={axis_idx} d=({dx*1000:.1f},{dy*1000:.1f})mm "
                f"rot={dtheta:.4f}rad col=({col[0]:+.1f},{col[1]:+.1f},{col[2]:+.1f})"
            )
    ...
    @staticmethod
    def _sfp_line_delta_correction(jacobian: np.ndarray, weighted_error: np.ndarray) -> np.ndarray:
        jac = np.asarray(jacobian, dtype=np.float64).reshape(3, 3)
        err = np.asarray(weighted_error, dtype=np.float64).reshape(3)
        try:
            delta = -_PIXEL_ALIGN_LAMBDA * np.linalg.pinv(jac, rcond=1e-3) @ err
        except np.linalg.LinAlgError:
            return np.zeros(3, dtype=np.float64)
        xy_norm = float(np.linalg.norm(delta[:2]))
        if xy_norm > _PIXEL_ALIGN_MAX_STEP_M:
            delta[:2] *= _PIXEL_ALIGN_MAX_STEP_M / xy_norm
        delta[2] = float(np.clip(delta[2], -_SFP_LINE_ALIGN_MAX_ROT_STEP_RAD, _SFP_LINE_ALIGN_MAX_ROT_STEP_RAD))
        return delta.astype(np.float64)

    def _run_sfp_line_align(
        self,
        get_observation,
        move_robot,
        send_feedback,
    ...
    ) -> tuple[bool, np.ndarray, np.ndarray, dict | None]:
        if self._target_plug_type != "sfp" and self._target_port_type != "sfp":
            return False, init_pos, init_quat, None

        obs_msg = get_observation()
        if obs_msg is None:
            return False, init_pos, init_quat, None
        meas = self._best_sfp_line_measurement(obs_msg)
        if meas is None:
            self.get_logger().info("SFP_LINE_ALIGN skipped: missing fresh plug/port alignment lines")
            send_feedback("SFP_LINE_ALIGN skipped reason=no_fresh_lines")
            return False, init_pos, init_quat, None
        ...
        pos = init_pos.copy()
        quat = init_quat.copy()
        last_meas: Optional[dict] = meas
        rotation_axis = np.asarray(insert_dir, dtype=np.float64).reshape(3)

        self.get_logger().info(
            "=== PHASE 1.6: SFP plug/port line convergence === "
            f"camera={camera} mid_err={meas['err_px']:.1f}px "
            f"angle_err={math.degrees(meas['angle_error_rad']):+.1f}deg "
            f"port={meas['port_line'].get('name', '?')} plug={meas['plug_line'].get('name', '?')}"
        )
        send_feedback(
            f"SFP_LINE_ALIGN start camera={camera} mid_err={meas['err_px']:.1f}px "
            f"angle={math.degrees(meas['angle_error_rad']):+.1f}deg"
        )
        ...
        for step in range(_PIXEL_ALIGN_MAX_STEPS):
            ...
            if obs_msg is None:
                time.sleep(_STEP_S)
                continue
            current = self._camera_sfp_line_measurement(camera, getattr(obs_msg, f"{camera}_image", None))
            if current is None:
                current = self._best_sfp_line_measurement(obs_msg)
                if current is None:
                    self.get_logger().info("SFP_LINE_ALIGN stopped: lost fresh plug/port lines")
                    return False, pos, quat, last_meas
                camera = str(current["camera"])
                jacobian = None

            last_meas = current
            err_px = float(current["err_px"])
            angle_abs = abs(float(current["angle_error_rad"]))
            if err_px <= _SFP_LINE_ALIGN_TOL_PX and angle_abs <= _SFP_LINE_ALIGN_TOL_RAD:
                stable_frames += 1
                if stable_frames >= _PIXEL_ALIGN_STABLE_FRAMES:
                    pos, quat, _, _ = self._get_tcp_state(obs_msg)
                    self.get_logger().info(
                        f"SFP_LINE_ALIGN complete step={step} mid_err={err_px:.1f}px "
                        f"angle={math.degrees(current['angle_error_rad']):+.1f}deg"
                    )
                    send_feedback(
                        f"SFP_LINE_ALIGN complete mid_err={err_px:.1f}px "
                        f"angle={math.degrees(current['angle_error_rad']):+.1f}deg"
                    )
                    return True, pos, quat, current
            else:
                stable_frames = 0

            if jacobian is None:
                jacobian = self._estimate_sfp_line_jacobian(
                    get_observation,
                    move_robot,
                    camera,
                    start_time,
                    current["weighted_error"],
                    rotation_axis,
                )
                if jacobian is None:
                    self.get_logger().info("SFP_LINE_ALIGN stopped: could not estimate local line Jacobian")
                    return False, pos, quat, current
                continue

            delta = self._sfp_line_delta_correction(jacobian, current["weighted_error"])
            step_xy = float(np.linalg.norm(delta[:2]))
            step_rot = abs(float(delta[2]))
            if step_xy < 1e-6 and step_rot < 1e-5:
                self.get_logger().info(
                    f"SFP_LINE_ALIGN stopped: tiny correction mid_err={err_px:.1f}px "
                    f"angle={math.degrees(current['angle_error_rad']):+.1f}deg"
                )
                break
            if total_xy + step_xy > _PIXEL_ALIGN_MAX_TOTAL_M or total_rot + step_rot > _SFP_LINE_ALIGN_MAX_TOTAL_ROT_RAD:
                self.get_logger().info(
                    "SFP_LINE_ALIGN stopped: correction cap reached "
                    f"xy={(total_xy + step_xy)*1000:.1f}mm rot={math.degrees(total_rot + step_rot):.1f}deg"
                )
                break

            before_norm = float(np.linalg.norm(current["weighted_error"]))
            obs_after, pos_after, quat_after = self._send_relative_line_delta(
                get_observation,
                move_robot,
                float(delta[0]),
                float(delta[1]),
            ...
            after = self._camera_sfp_line_measurement(camera, getattr(obs_after, f"{camera}_image", None))
            after_norm = float(np.linalg.norm(after["weighted_error"])) if after is not None else float("inf")
            if after_norm > before_norm + 3.0:
                jacobian = None

            if step < 3 or step % 5 == 0:
                after_mid = float(after["err_px"]) if after is not None else float("inf")
                after_ang = float(after["angle_error_rad"]) if after is not None else float("inf")
                self.get_logger().info(
                    "SFP_LINE_ALIGN "
                    f"step={step} camera={camera} mid={err_px:.1f}->{after_mid:.1f}px "
                    f"ang={math.degrees(current['angle_error_rad']):+.1f}->{math.degrees(after_ang):+.1f}deg "
                    f"dxy=({delta[0]*1000:+.2f},{delta[1]*1000:+.2f})mm "
                    f"drot={math.degrees(delta[2]):+.2f}deg"
                )
                send_feedback(
                    f"SFP_LINE_ALIGN step={step} mid={err_px:.1f}->{after_mid:.1f}px "
                    f"angle={math.degrees(current['angle_error_rad']):+.1f}->{math.degrees(after_ang):+.1f}deg"
                )
    ...
    def insert_cable(
        ...
        **kwargs,
        ...
        if aligned_for_insert and pos is not None and self._now() - start < self.time_limit_s - 5.0:
            ...
            is_sfp_task = self._target_plug_type == "sfp" or self._target_port_type == "sfp"
            if is_sfp_task and self._now() - start < self.time_limit_s - 10.0:
                line_ok, line_pos, line_quat, line_meas = self._run_sfp_line_align(
                    get_observation,
                    move_robot,
                    send_feedback,
                    start,
                ...
                if line_pos is not None and line_quat is not None:
                    pos, quat = line_pos, line_quat
                if line_meas is not None:
                    self.get_logger().info(
                        "SFP line align result: "
                        f"ok={line_ok} mid_err={line_meas['err_px']:.1f}px "
                        f"angle={math.degrees(line_meas['angle_error_rad']):+.1f}deg"
                    )
