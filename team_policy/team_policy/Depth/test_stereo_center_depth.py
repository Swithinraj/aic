...
class StereoCenterDepthPublisher(Node):
    def __init__(self):
        super().__init__("test_stereo_center_depth")
        self._depth = StereoCenterDepth(self, publish_outputs=True)
        self._status_timer = self.create_timer(2.0, self._tick)

        self.get_logger().info("Depth Anything V2 Metric Indoor Base three-camera depth publisher started")
    ...
    def _tick(self):
        ...
        for camera_name in ("left", "center", "right"):
            ...
            if valid.any():
                ...
                depth_mean = float(result.depth_meters[valid].mean())
                self.get_logger().info(
                    f"camera={camera_name} source={result.sources_used} coverage={result.coverage:.4f} depth_min_m={depth_min:.4f} depth_mean_m={depth_mean:.4f} depth_max_m={depth_max:.4f}"
                )
            else:
                self.get_logger().warn(f"camera={camera_name} has no valid metric depth pixels")
        if missing:
