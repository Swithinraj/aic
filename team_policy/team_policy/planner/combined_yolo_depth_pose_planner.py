...
class CombinedYoloDepthPosePlanner(YoloV12MultiCameraDetector):
    """YOLO canonical detections + debug-only metric depth anchor TFs.

    These approximate depth anchors are published for RViz/debugging only.
    Alignment/control in RunHybrid must use image-space detections plus
    robot/camera TF, not these object poses.
    """
    ...
    def __init__(self):
        ...
        self._debug_fallback_depth = self._env_bool("YOLO_DEPTH_POSE_DEBUG_FALLBACK_ENABLE", False)
        ...
        self.get_logger().info(
            "Combined planner node started: YOLO + debug metric depth anchors, "
            f"TF camera={self._tf_camera}, base_frame={self._base_frame}, "
            f"debug_fallback_depth={self._debug_fallback_depth}"
        )
    ...
    def _attach_depth_anchor_poses(self, camera_name: str, detections: List[Dict]) -> List[Dict]:
        ...
        for det in detections:
            ...
            det2["pose_source"] = "debug_depth_anchor_approx"
            det2["control_valid"] = False
            out.append(det2)
        return out
    ...
    def _backproject_anchor(self, info: CameraInfo, depth: Optional[np.ndarray], anchor_uv: np.ndarray) -> Optional[np.ndarray]:
        ...
        if Z is None:
            if not self._debug_fallback_depth:
                return None
    ...
    def _publish_fused_from_instance_obs(self, instance_obs: Dict[str, List[Dict]]) -> None:
        ...
        for name in sorted(instance_obs.keys(), key=self._tf_sort_key):
            ...
            if pose_base is None:
                continue
            tf_name = "debug_yolo_depth_" + self._tf_safe_name(name)
            anchor_keys = (
                "servo_anchor_valid",
                "servo_anchor_source",
            ...
            fused_records.append(
                {
                    ...
                    "camera_frame_id": camera_frame_id,
                    "pose_source": "debug_depth_anchor_approx",
                    "control_valid": False,
                    **anchors,
                }
            )
