...
### Step 4: Run an Planning specific debugger scripts

To get the depth estimate images as a topic which could be subscribed for later use. To get the depth estimate images as a topic which could be subscribed for later use.


With the simulation environment running (Step 2), run the following policy:
```bash
cd ~/ws_aic/src/aic
pixi run ros2 run team_policy test_stereo_center_depth
```

To Move the robotic arm with an interactive marker and to view the planned trajectory.


And again With the simulation environment running (Step 2), run the following policy:
```bash
cd ~/ws_aic/src/aic
